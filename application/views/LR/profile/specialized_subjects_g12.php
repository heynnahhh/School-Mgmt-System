<?php include '/../templates/header_profile.php';?>

<?php include '/../templates/profile_nav.php';?>

      <!-- **********************************************************************************************************************************************************
      MAIN CONTENT
      *********************************************************************************************************************************************************** -->
      <!--main content start-->

       <section id="main-content">
          <section class="wrapper">

                <div class="row">
                  <div class="col-lg-9 main-chart">
                   <h3><i class="fa fa-angle-right"></i> Senior High School Resources</h3>
                    <hr></i>
                    <div class="row mt">
                      <div class="col-md-12">
                        <div class="white-panel pn">
                          <div class="panel-heading">
                            <div class="pull-left"><h3></i><h3> Grade 12 <i class="fa fa-angle-right"></i> Specialized Subjects by Strands</h3></div>
                            <br>
                          </div>

                          <div class="custom-check goleft mt">
        										 <table id="todo" class="table table-hover custom-check">
        										  <tbody>
                                <?php foreach ($shs_ss_strand as $shs_ss_strand) {?>
        											<tr>
        												<td>
                                    <?php echo '<a href="';?>
                                      <?php echo base_url('index.php/lr/profile/Grade%2012/seniorhs/specialized_subjects/');?><?php echo $shs_ss_strand->strand;?>
                                    <?php echo '">';?>
                                      <?php echo $shs_ss_strand->strand;?>
                                    <?php echo '</a>';?>
                                    <?php echo '<div class="pull-right hidden-phone">';?>
                                      <?php echo '<span class="badge bg-important">35</span>';?>
                                    <?php echo '</div>';?>
                                <?php }?>
        													</div>
        												</td>
        											</tr>
        										  </tbody>
        									  </table>

                          </div><!-- /table-responsive -->
                        </div><!--/ White-panel -->
                      </div><! --/col-md-12 -->
                    </div><! -- row -->
                  </div>

       <?php include '/../templates/latest_addition.php';?>

                  </div><!-- /col-lg-9 END SECTION MIDDLE -->


              </div><! --/row -->
          </section>
      </section>
      <!--main content end-->

      <?php include '/../templates/footer.php';?>

      <?php include '/../templates/ajax.php';?>
